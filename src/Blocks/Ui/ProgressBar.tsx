...
const ProgressBar = ({ progress }: { progress: number }) => {
    ...
    return (
        <div className="min-h-2 border w-[70%] mx-auto mt-12 mb-5 relative rounded-lg">
            <span className=" bg-gray-300 px-2 py-1 text-xs font-bold rounded-sm absolute bottom-4 duration-300 min-w-10 text-center" style={{ right: `${(100 - progress) - 4}%` }}>{progress.toFixed(1)}% {arrow}</span>
